...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const existingUserList = (existingUsers as unknown as { users?: Array<{ email?: string | null }> })
      ?.users;
    const userExists = existingUserList?.some((u) => (u.email ?? '').toLowerCase() === email.toLowerCase());
    if (userExists) {
