...
import { exportQuotationsToCSV, exportQuotationsToPDF } from '@/lib/exportUtils';
...
const ManagerQuotations = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      return (
        ...
        (r.quotation_id || '').toLowerCase().includes(query)
      );
    });
  }, [rows, q]);
  ...
  return (
    <>
      <Helmet>
        <title>Manager Quotations - Zervitra</title>
        ...
        <div className="container mx-auto px-4">
          <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
              ...
              <p className="text-muted-foreground">View all quotations you created for your clients.</p>
            </div>
          ...
          <section className="premium-glass rounded-3xl p-6 border border-white/10 space-y-4">
            ...
            <ExpandedQuotationsTable
              quotations={filtered}
              onExportCSV={() => exportQuotationsToCSV(filtered)}
              onExportPDF={() => exportQuotationsToPDF(filtered)}
            />
          </section>
        </div>
      </main>
